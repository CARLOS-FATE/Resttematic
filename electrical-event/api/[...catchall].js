...
app.post('/api/login', async (req, res) => {
    try {
        ...
        jwt.sign(payload, secret, { expiresIn: '8h' }, (err, token) => { // Aumentado a 8 horas
            if (err) throw err;
...
app.put('/api/tables/:id/status', auth(['mesero', 'administrador', 'dueno']), async (req, res) => {
    try {
        ...
        res.status(400).json({ message: 'Error al actualizar el estado de la mesa.' });
    }
});
...
app.put('/api/orders/:id/status', auth(['cocinero']), async (req, res) => {
    try {
        ...
        res.status(400).json({ message: 'Error al actualizar el estado del pedido.' });
    }
});
...
app.put('/api/orders/:id/paid', auth(['caja']), async (req, res) => {
    try {
        ...
        res.status(400).json({ message: 'Error al actualizar el estado de pago.' });
    }
});

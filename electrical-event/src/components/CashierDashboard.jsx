...
const PaymentForm = ({ orderId, onMarkAsPaid, setError }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="mt-4 p-4 bg-gray-50 rounded-b-lg border-t border-gray-200">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <select
                    ...
                    className="w-full sm:w-auto flex-grow p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 text-gray-700"
                >
    ...
    <>
       <div className="bg-gray-50 min-h-screen">
                <div className="container mx-auto p-4 md:p-6">
                    <h1 className="text-3xl md:text-4xl font-bold mb-6 text-center text-gray-800">Panel de Caja</h1>
                    ...
                    <div className="mb-6 flex justify-center border-b-2">
                        ...
                        <button onClick={() => setView('reservations')} className={`py-2 px-6 font-semibold ${view === 'reservations' ? 'border-b-4 border-blue-500 text-blue-600' : 'text-gray-500'}`}>
                            Reservas Pendientes
                        </button>
                    </div>
          {error && <p className="bg-red-100 text-red-700 p-3 rounded-md mb-4 text-center">{error}</p>}
           ...
           {view === 'orders' && (
                        <>
                            <h2 className="text-2xl font-semibold mb-4 text-gray-700">Pedidos Pendientes de Pago</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                                {/* Usamos la variable correcta 'pendingOrders' */}
                                {pendingOrders.length > 0 ? (
                                    pendingOrders.map(order => (
                                        <div key={order._id} className="rounded-lg shadow-md bg-blue-50 transition-shadow hover:shadow-xl">
                            ...
                            <h2 className="text-2xl font-semibold mb-4 text-gray-700 mt-8 border-t pt-8">Pedidos Ya Pagados</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                {paidOrders.length > 0 ? (
                                    paidOrders.map(order => (
                                        <div key={order._id} className="rounded-lg shadow-md bg-white transition-shadow hover:shadow-xl">
                                            <OrderCard order={order} onGenerateReceipt={handleShowReceipt} />
                                        </div>
                                    ))
                                ) : (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {reservations.length > 0 ? (
                                reservations.map(res => (
                                    <div key={res._id} className="bg-white p-6 rounded-lg shadow-md text-gray-800 flex flex-col justify-between">
                                        <div>
                                            ...
                                            <p className="mt-2 text-red-600 font-semibold">Estado: Pendiente de Pago</p>
                                        </div>
                                        <button
                                           onClick={() => {
                   ...
                   {selectedReservation && (
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                            <div className="bg-white p-6 rounded-lg shadow-xl text-gray-800 w-full max-w-md">
                                ...
                                <form onSubmit={handleConfirmReservationPayment} className="space-y-4">
                                    <input type="number" placeholder="Monto Pagado (S/)" onChange={e => setPaymentDetails({...paymentDetails, montoPagado: e.target.value})} className="w-full p-2 border rounded" required />
                                    <input type="text" placeholder="ID de Transacción / Comprobante" onChange={e => setPaymentDetails({...paymentDetails, comprobantePago: e.target.value})} className="w-full p-2 border rounded" required />
                                    <textarea placeholder="Notas adicionales..." onChange={e => setPaymentDetails({...paymentDetails, notasCajero: e.target.value})} className="w-full p-2 border rounded"></textarea>
                                    <div className="flex justify-end gap-4">
                                        <button type="button" onClick={() => setSelectedReservation(null)} className="bg-gray-300 py-2 px-4 rounded">Cancelar</button>
                                        <button type="submit" className="bg-green-500 text-white py-2 px-4 rounded">Confirmar</button>
                                    </div>
                                </form>
                            </div>
                        </div>

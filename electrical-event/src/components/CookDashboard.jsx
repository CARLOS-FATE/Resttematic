  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      <div className="container mx-auto p-4 md:p-6">
        <h1 className="text-3xl md:text-4xl font-bold mb-6 text-center text-gray-800">Comandas de Cocina</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-4 rounded-lg shadow-md">
            <h2 className="text-xl font-bold mb-4 text-red-600 border-b pb-2">Pendientes ({pendingOrders.length})</h2>
          ...
          <div className="bg-white p-4 rounded-lg shadow-md">
            <h2 className="text-xl font-bold mb-4 text-blue-600 border-b pb-2">En Preparación ({preparingOrders.length})</h2>
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
              {preparingOrders.length > 0 ? (
                ...
                <p className="text-center text-gray-500 py-4">No hay pedidos en preparación.</p>
              )}
            </div>
          </div>
...
const OrderCard = ({ order, actionText, actionHandler, buttonColor }) => (
  <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-sm">
    <div className="flex justify-between items-center mb-3">
        <h3 className="text-lg font-bold text-gray-800">Mesa #{order.numeroMesa}</h3>
    ...
    <ul className="mb-4 space-y-2">
      {order.items.map(item => (
        <li key={item.menuItemId} className="text-gray-800 text-lg">
          <span className="font-bold text-blue-600">{item.cantidad}x</span> {item.nombre}
        </li>
      ))}
    </ul>
    <button

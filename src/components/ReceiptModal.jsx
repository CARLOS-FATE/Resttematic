...
const ReceiptModal = ({ order, documentType, onClose }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
      <div className="receipt-container bg-white dark:bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-md relative text-gray-900 dark:text-gray-100 transition-colors duration-300">
        {/* Contenido del comprobante */}
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold">{restaurant.name}</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400">{restaurant.address}</p>
          {isBoleta && <p className="text-sm text-gray-600 dark:text-gray-400">RUC: {restaurant.ruc}</p>}
        </div>

        <div className="border-y-2 border-dashed border-gray-300 dark:border-gray-600 py-4 mb-4">
        ...
        {/* Totales */}
        <div className="border-t-2 border-dashed border-gray-300 dark:border-gray-600 pt-4 mt-4 text-sm">
          {isBoleta && (
            <>
        ...
        <div className="no-print mt-8 flex justify-center gap-4">
          ...
          <button onClick={onClose} className="bg-gray-300 dark:bg-gray-600 text-gray-800 dark:text-white font-bold py-2 px-6 rounded hover:bg-gray-400 dark:hover:bg-gray-500 transition">Cerrar</button>
        </div>
      </div>
